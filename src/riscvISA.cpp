...
const char *riscvNamesOP[8] = {"ADD", "SLL", "CMPLT", "CMPLTU",
                               "XOR", "",    "OR",    "AND"};
const char *riscvNamesOPI[8] = {"ADDi", "SLLi", "SLTi", "CMPLTUi",
                                "XORi", "SRLi", "ORi",  "ANDi"};
const char *riscvNamesOPW[8] = {"ADDW", "SLLW", "", "", "", "SRW", "", ""};
const char *riscvNamesOPIW[8] = {"ADDWi", "SLLWi", "", "", "", "SRi", "", ""};
const char *riscvNamesLD[8] = {"LDB",  "LDH",  "LDW", "LDD",
                               "LDBU", "LDHU", "LDWU"};
const char *riscvNamesST[8] = {"STB", "STH", "STW", "STD"};
const char *riscvNamesBR[8] = {"BEQ", "BNE", "",     "",
                               "BLT", "BGE", "BLTU", "BGEU"};
const char *riscvNamesMUL[8] = {"MPYLO", "MPYHI", "MPYHI", "MPYHI",
                                "DIVHI", "DIVHI", "DIVLO", "DIVLO"};
const char *riscvNamesCSR[8] = {"",       "CSRRW",  "CSRRS",  "CSRRC",
                                "CSRRWI", "CSRRSI", "CSRRCI", ""};
const char *riscvNamesAMO[29] = {"AMO_ADDW", "AMO_SWAPW",  "AMO_LRW",  "AMO_SCW",
                                "AMO_XORW", "", "", "", "AMO_ORW", "", "", "", "AMO_ANDW",
                                "", "", "", "AMO_MINW", "", "", "", "AMO_MAXW",
                                "", "", "", "AMO_MINWU", "", "", "", "AMO_MAXWU"};
...
std::string printDecodedInstrRISCV(unsigned int oneInstruction) {
  ...
  char rs3 = ((oneInstruction >> 27) & 0x1f); // funct5
  ...
  unsigned short imm12_S =
      ((oneInstruction >> 20) & 0xfe0) + ((oneInstruction >> 7) & 0x1f);

  short imm12_I_signed = (imm12_I >= 2048) ? imm12_I - 4096 : imm12_I;
  short imm12_S_signed = (imm12_S >= 2048) ? imm12_S - 4096 : imm12_S;

  short imm13 =
      ((oneInstruction >> 19) & 0x1000) + ((oneInstruction >> 20) & 0x7e0) +
      ((oneInstruction >> 7) & 0x1e) + ((oneInstruction << 4) & 0x800);
  ...
  unsigned int imm21_1 =
      (oneInstruction & 0xff000) + ((oneInstruction >> 9) & 0x800) +
  ...
  if (opcode == RISCV_OPI &&
      (funct3 == RISCV_OPI_SLLI || funct3 == RISCV_OPI_SRI))
    funct7 = funct7 & 0x3f;
  ...
  case RISCV_OPI:
    if (funct3 == RISCV_OPI_SRI)
      if (funct7 == RISCV_OPI_SRI_SRLI)
        stream << "SRLi r" << (int)rd << " = r" << (int)rs1 << ", "
               << (int)shamt << " ";
      else // SRAI
        stream << "SRAi r" << (int)rd << " = r" << (int)rs1 << ", "
               << (int)shamt << " ";
    else if (funct3 == RISCV_OPI_SLLI) {
      stream << riscvNamesOPI[funct3];
      stream << " r" << (int)rd << " = r" << (int)rs1 << ", " << (int)shamt
             << " ";
    } else {
      stream << riscvNamesOPI[funct3];
    ...
    if (funct7 == 1) {
      ...
      if (funct3 == RISCV_OP_ADD)
        if (funct7 == RISCV_OP_ADD_ADD)
          stream << "ADD r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
        else
          stream << "SUB r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
      else if (funct3 == RISCV_OP_SR)
        if (funct7 == RISCV_OP_SR_SRL)
          stream << "SRL r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
        else // SRA
          stream << "SRA r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
      else {
    ...
    if (funct7 == 1) {
      ...
      if (funct3 == RISCV_OP_ADD)
        if (funct7 == RISCV_OPW_ADDSUBW_ADDW)
          stream << "ADDW r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
        else
          stream << "SUBW r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
      else if (funct3 == RISCV_OPW_SRW)
        if (funct7 == RISCV_OPW_SRW_SRLW)
          stream << "SRLW r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
        else // SRAW
          stream << "SRAW r" << (int)rd << " = r" << (int)rs1 << ", r"
                 << (int)rs2 << " ";
      else {
  ...
  case RISCV_SYSTEM:
    ...
    break;
  case RISCV_AMO:
    if(funct3 == 2){ // width must equal to 2
      stream << riscvNamesAMO[rs3];
      stream << " r" << (int)rd << " r" << (int)rs1 << " r" << (int) rs2;
    } else {
      stream << "Error AMO funct3 AMO" << " ";
    }
    break;
  case RISCV_MISC_MEM:
    stream << "FENCE";
