...
bool getTest(std::istream &is, std::string &message, unsigned int &instruction,
             unsigned int &numberOfCycles, struct processorState &initialState,
             struct processorState &finalState) {
...
int main(int argc, char **argv) {
  ...
  while (infile.peek() != EOF) {
    ...
    getTest(infile, message, instruction, numberOfCycles, initialState,
            finalState);
    std::cerr << message << '\n';
    numberOfCycles = 35;
    ...
    core.im = new CacheMemory<4,16,64>(new IncompleteMemory<4>(im, &reservationAMO), false);
    core.dm = new CacheMemory<4,16,64>(new IncompleteMemory<4>(dm, &reservationAMO), true);
    ...
    for (int oneCycle = 0; oneCycle < numberOfCycles; oneCycle++) {
      ...
      doCycle(core, 0, clint.ir_timer[0], clint.ipi[0]);
    }
    ...
    for (int oneReg = 0; oneReg < 32; oneReg++) {
      if (core.regFile[oneReg] != finalState.regs[oneReg]) {
        std::cerr << std::endl
                  << "\t Error: value for reg " << oneReg << " is "
                  << core.regFile[oneReg] << " but should be "
    ...
    if (dm[finalState.address >> 2] != finalState.value) {
      std::cerr << std::endl
                << "\t Error: memory value at address " << finalState.address
                << " is " << dm[finalState.address] << " but should be "
                << finalState.value << std::endl;
      worked = false;
    }
